...
public class SalesReportGUI extends Application {;
	...
	public void readFile() throws FileNotFoundException {
		...
		if ((selectedFile = chooser.showOpenDialog(null)) != null) {
			// Read the file
			sales = FileUtility.readFile(selectedFile);
		}
		//clear table
		clearTable();
		...
		for(row=0;row<sales.length;row++)
		{
			total = FileUtility.getRowTotal(sales, row);
		...
		//find lowest in each column
			...
			for(col=0;col<columns;col++)
			{
				lowest = FileUtility.getLowestInColumn(sales, col);
				highest = FileUtility.getHighestInColumn(sales, col);
	...
	public void copyFile() throws FileNotFoundException
	{
		...
		if ((selectedFile = chooser.showSaveDialog(null)) != null) {
			// Read the file
			FileUtility.writeToFile(sales,selectedFile);
		}
	}
	...
	public void start(Stage stage) {
		...
		Label titleLbl = new Label("Company Sales Report\n");
		...
		dataPane.add(new Label("Chicken Nuggets"), 1, 0);
		dataPane.add(new Label("Fries"), 2, 0);
		dataPane.add(new Label("Burgers"), 3, 0);
		dataPane.add(new Label("Chicken Sandwich"), 4, 0);
		dataPane.add(new Label("Soft Drinks"), 5, 0);
		dataPane.add(new Label("Smoothies"), 6, 0);
		dataPane.add(new Label("Total"), 7, 0);
		dataPane.add(new Label("Bonus"), 8, 0);
		...
		dataPane.add(new Label("Location #1"), 0, 1);
		dataPane.add(new Label("Location #2"), 0, 2);
		dataPane.add(new Label("Location #3"), 0, 3);
		dataPane.add(new Label("Location #4"), 0, 4);
		dataPane.add(new Label("Location #5"), 0, 5);
		dataPane.add(new Label("Location #6"), 0, 6);
